    ...
    return (
      <div style={s.page}>
        <div style={s.card}>
          ...
          {isIOS ? (
            <>
              <p style={{ ...s.sub, marginBottom: 24 }}>iPhoneは下の手順で登録してください。</p>
              <div style={step.list}>
                {[
                  ["1", "Safariで開く", "今のブラウザはNG。Safariアプリで開いてください。"],
                  ["2", "「ホーム画面に追加」する", "画面下の共有ボタン（四角に↑）をタップ → 「ホーム画面に追加」"],
                  ["3", "ホーム画面のアイコンから起動", "追加されたアイコンをタップして起動し、名前を入力して登録完了！"],
                ].map(([n, t, d]) => (
            ...
            <>
              <p style={{ ...s.sub, marginBottom: 24 }}>Androidの方はChromeで開いて通知を許可してください。</p>
            </>
          )}
        </div>
      </div>
